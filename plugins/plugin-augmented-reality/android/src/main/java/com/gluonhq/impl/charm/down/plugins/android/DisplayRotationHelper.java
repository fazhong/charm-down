...
/**
 * Helper to track the display rotations. In particular, the 180 degree
 * rotations are not notified by the onSurfaceChanged() callback, and thus they
 * require listening to the android display events.
 */
public final class DisplayRotationHelper implements DisplayListener {
    ...
    /**
     * Constructs the DisplayRotationHelper but does not register the listener
     * yet.
     *
     ...
     * #onSurfaceChanged(int, int)} call or by {@link #onDisplayChanged(int)}
     * system callback. This function should be called explicitly before each
     * call to {@link Session#update()}. This function will also clear the
     * 'pending update' (viewportChanged) flag.
     *
     * @param session the {@link Session} object to update if display geometry
     * changed.
     */
    ...
    /**
     * Returns the current rotation state of android display. Same as
